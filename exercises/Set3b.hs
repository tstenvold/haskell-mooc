...
buildList start count end = start : next
  where
    next = buildList start (count - 1) end
...
summer :: [Int] -> [Int]
summer [] = []
summer (x:xs) = sumList [1 .. x] : summer xs
...
mymaximum f i (x:xs)
  | i `f` x = mymaximum f i xs
  | otherwise = mymaximum f x xs
...
maybeMap f [] = []
maybeMap f (x:xs) = reduce (f x) xs
  where
    reduce (Just y) xs = y : maybeMap f xs
    reduce Nothing xs = maybeMap f xs
