...
travel _ (Time 0.0) = Distance 0.0
...
member x (Set b) = x `elem` b

-- add a member to a set
add :: (Ord a, Eq a) => a -> Set a -> Set a
add x (Set b) = Set (sort d)
  where
    d =
      if member x (Set b)
        then b
...
step Sugar AddFlour = SugFlo
step Flour AddSugar = SugFlo
step SugFlo Mix = Mixed
...
average (a :| []) = a
average (a :| as) = sum (a : as) / fromIntegral (length as + 1)
...
passwordAllowed pass (And a b) =
  passwordAllowed pass a && passwordAllowed pass b
